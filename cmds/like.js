...
module.exports.run = async (client, message, args) => {
    try {
        ...
        if (rUser.id == message.author.id) { embed.setDescription(msgs[1]); return message.channel.send(embed); };
        if (client.profile.fetch(`lvl_${message.author.id}`) < 10) { embed.setDescription(msgs[2]); return message.channel.send(embed); }
        for (let i = 0; i < liked.length; await i++) {
            if (liked[i] == rUser.id) { embed.setDescription(msgs[3]); return message.channel.send(embed); }
        }
