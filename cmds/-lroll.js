...
module.exports.run = async (client, message, args) => {
    try {
        ...
        if (!args[0] || !isNumeric(Math.floor(parseInt(args[0])))) {
            embed.setDescription(casino[3]);
            return message.channel.send(embed);
        }
        if (coins <= Math.floor(parseInt(args[0]))) {
            embed.setDescription(`${noMoney} **${coins}**`);
            return message.channel.send(embed);
        }
        if (Math.floor(parseInt(args[0])) <= 10) {
            embed.setDescription(`${casino[2]} 10`);
            return message.channel.send(embed);
        }
        rand2 + 13;
        ...
        if (rand <= rand2) {
            ...
            client.lprofile.subtract(`coins_${message.author.id}_${message.guild.id}`, Math.floor(parseInt(args[0])));
            return message.channel.send(embed);
        }
        if (rand > rand2) {
