...
module.exports.run = async (client, message, args) => {
    try {
        ...
        if (!args[0]) {
            ...
            if (coins < clancost) { clanemb.setDescription(`${msgs[5]} **${clancost.toLocaleString()}$**`); return client.send(clanemb); }
            if (clan != null) { clanemb.setDescription(`${msgs[6]}`); return client.send(clanemb); }
            if (!clanname) { clanemb.setDescription(`${msgs[7]}`); return client.send(clanemb); }
            ...
            if (clan != null) { clanemb.setDescription(`${msgs[6]}`); return client.send(clanemb); }
            if (!clanname) { clanemb.setDescription(`${msgs[10]}`); return client.send(clanemb); }
            if (client.clan.fetch(`${clanname}_admin`) === null) { clanemb.setDescription(`${msgs[11]}`); return client.send(clanemb); }
            let bans = client.clan.fetch(`${clanname}_bans_${userid}`);
            if (bans != null) { clanemb.setDescription(`${msgs[12]}`); return client.send(clanemb); }
            client.profile.set(`clan_${userid}`, clanname)
            client.clan.add(`${clanname}_members`, 1);
            clanemb.setDescription(`${msgs[13]}`);
            return client.send(clanemb);
            ...
            let rUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[1]));
            if (clan === null) { clanemb.setDescription(`${msgs[1]}`); return client.send(clanemb); }
            if (!args[1]) { clanemb.setDescription(noUser); return client.send(clanemb); }
            if (!rUser) { clanemb.setDescription(noUser); return client.send(clanemb); }
            let cal = client.clan.fetch(`${clan}_admin`);
            if (cal != message.author.id) { clanemb.setDescription(`${msgs[22]}`); return client.send(clanemb); }
            if (rUser.id == message.author.id) { clanemb.setDescription(`${msgs[23]}`); return client.send(clanemb); }
            if (client.clan.fetch(`${clan}_bans_${rUser.id}`) != null) { clanemb.setDescription(`${msgs[24]}`); return client.send(clanemb); }
            ...
            if (banned === null) { clanemb.setDescription(`${msgs[26]}`); return client.send(clanemb); }
            ...
            if (clan === null) { clanemb.setDescription(`${msgs[1]}`); return client.send(clanemb); }
            let cal = client.clan.fetch(`${clan}_admin`);
            if (cal == message.author.id) { clanemb.setDescription(`${msgs[28]}`); return client.send(clanemb); }
            client.clan.subtract(`${clan}_members`, 1);
            ...
            if (!args[1]) { clanemb.setDescription("Укажите название клана/Enter the name of the clan"); return client.send(clanemb); }
            if (client.clan.fetch(`${clanname}_money`) == null) { clanemb.setDescription(`${msgs[11]}`); return client.send(clanemb); }
