...
module.exports.run = async (client, message, args) => {
    try {
        ...
        let clan = client.profile.fetch(`clan_${userid}`)
        let coins = client.profile.fetch(`coins_${userid}`)
        let clancost = 250000;
        ...
        if (!args[0]) {
            if (client.clan.fetch(`${clan}_money`) === null) { client.profile.delete(`clan_${userid}`); client.clan.delete(`${clan}_bans_${userid}`) }
            clan = await client.profile.fetch(`clan_${userid}`)
            ...
            if (coins < clancost) { clanemb.setDescription(`${msgs[5]} **${clancost.toLocaleString()}$**`); return message.channel.send(clanemb); }
            if (clan != null) { clanemb.setDescription(`${msgs[6]}`); return message.channel.send(clanemb); }
            if (!clanname) { clanemb.setDescription(`${msgs[7]}`); return message.channel.send(clanemb); }
            let created = client.clan.fetch(clanname);
            if (created === null) {
                clanemb.setDescription(`${msgs[8]} **${clanname}**'`)

                client.profile.subtract(`coins_${userid}`, clancost)
                client.profile.set(`clan_${userid}`, clanname)
            ...
            if (clan != null) { clanemb.setDescription(`${msgs[6]}`); return message.channel.send(clanemb); }
            if (!clanname) { clanemb.setDescription(`${msgs[10]}`); return message.channel.send(clanemb); }
            if (client.clan.fetch(`${clanname}_admin`) === null) { clanemb.setDescription(`${msgs[11]}`); return message.channel.send(clanemb); }
            let bans = client.clan.fetch(`${clanname}_bans_${userid}`);
            if (bans != null) { clanemb.setDescription(`${msgs[12]}`); return message.channel.send(clanemb); }
            client.profile.set(`clan_${userid}`, clanname)
            ...
            let rUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[1]));
            if (clan === null) { clanemb.setDescription(`${msgs[1]}`); return message.channel.send(clanemb); }
            if (!args[1]) { clanemb.setDescription(noUser); return message.channel.send(clanemb); }
            if (!rUser) { clanemb.setDescription(noUser); return message.channel.send(clanemb); }
            let cal = client.clan.fetch(`${clan}_admin`);
            if (cal != message.author.id) { clanemb.setDescription(`${msgs[22]}`); return message.channel.send(clanemb); }
            if (rUser.id == message.author.id) { clanemb.setDescription(`${msgs[23]}`); return message.channel.send(clanemb); }
            if (client.clan.fetch(`${clan}_bans_${rUser.id}`) != null) { clanemb.setDescription(`${msgs[24]}`); return message.channel.send(clanemb); }
            ...
            if (banned === null) { clanemb.setDescription(`${msgs[26]}`); return message.channel.send(clanemb); }
            ...
            if (clan === null) { clanemb.setDescription(`${msgs[1]}`); return message.channel.send(clanemb); }
            let cal = client.clan.fetch(`${clan}_admin`);
            if (cal == message.author.id) { clanemb.setDescription(`${msgs[28]}`); return message.channel.send(clanemb); }
            client.clan.subtract(`${clan}_members`, 1);
            ...
            client.profile.delete(`clan_${userid}`)
            ...
            if (!args[1]) { clanemb.setDescription("Укажите название клана/Enter the name of the clan"); return message.channel.send(clanemb); }
            if (client.clan.fetch(`${clanname}_money`) == null) { clanemb.setDescription(`${msgs[11]}`); return message.channel.send(clanemb); }
