...
module.exports.run = async (client, message, args) => {
    try {
        ...
        switch (args[0]) {
            case 'send':
                if (!args[1]) { embed.setColor('#e22216'); embed.setDescription(noUser); return client.send(embed); }
                let rUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[1]));
                if (!rUser) { embed.setColor('#e22216'); embed.setDescription(noUser); return client.send(embed); }

                if (client.profile.fetch(`partner_${message.author.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[4]); return client.send(embed); }
                if (rUser.id == message.author.id) { embed.setColor('#e22216'); embed.setDescription(msgs[5]); return client.send(embed); }
                if (client.profile.fetch(`partner_${rUser.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[6]); return client.send(embed); }
                if (client.profile.fetch(`sended_${rUser.id}`) != null || client.profile.fetch(`sender_${rUser.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[7]); return client.send(embed); }
                if (client.profile.fetch(`sended_${message.author.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[8]); return client.send(embed); }
                if (client.profile.fetch(`sender_${message.author.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[9]); return client.send(embed); }
            ...
            case 'accept':
                if (client.profile.fetch(`sender_${message.author.id}`) == null || client.profile.fetch(`partner_${message.author.id}`) != null) { embed.setColor('#e22216'); embed.setDescription(msgs[11]); return client.send(embed); }
                ...
                break;
            case `divorce`:
                if (client.profile.fetch(`partner_${message.author.id}`) == null) { embed.setColor('#e22216'); embed.setDescription(msgs[13]); return client.send(embed); }
                let partner = client.users.get(client.profile.fetch(`partner_${message.author.id}`))
                ...
                break;
            case 'cancel':
                if (client.profile.fetch(`sender_${message.author.id}`) == null && client.profile.fetch(`sended_${message.author.id}`) == null) { embed.setColor('#e22216'); embed.setDescription(msgs[14]); return client.send(embed); }
